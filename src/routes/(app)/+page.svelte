<script lang="ts">
  ...
  import { cancelOllamaRequest } from '$lib/apis/ollama';
  import { generateTitle } from '$lib/apis/openai';
  ...
  const analysisimageinfo = async (messageId: string) => {
    ...
    if (userMessage.files) {
      if (
        ...
        (userMessage.files[0].type == 'image' || (userMessage.files[0]?.image ?? []).length > 0)
      ) {
  ...
  const generateSearchChatKeyword = async (userPrompt: string) => {
    if ($settings?.title?.auto ?? true) {
      ...
      const title = await generateSearchKeyword(localStorage.token, send_messages, userPrompt, $deApiBaseUrl?.url);
      return title;
    } else {
      return `${userPrompt}`;
    }
...
<div
  class="min-h-screen max-h-screen {$showSidebar ? 'md:max-w-[calc(100%-246px)]' : ''} w-full max-w-full flex flex-col"
>
