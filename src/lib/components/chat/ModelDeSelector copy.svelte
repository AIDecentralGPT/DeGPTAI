...
<DropdownMenu.Root bind:open={show}>
  ...
  <DropdownMenu.Content keepOpenOnContentChange={true} class="z-[90] {$mobile ? `w-11/12`: `w-[500px]`}  justify-start border bg-white dark:bg-gray-850 rounded-lg p-3 ml-2"
        ...
        <div class="px-5">
          <DropdownMenu.CheckboxItem>
            <button class="flex flex-row items-center w-full region-text-color my-2 select-none data-[highlighted]:bg-muted"
              on:click = { async (e) => {
                await assignDeUrl("North America");
                e.stopPropagation();
              }}>
