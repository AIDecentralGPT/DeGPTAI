<script lang="ts">
  ...
  export let selDeUrl: string = "";
  // selDeUrl = $DE_API_BASE_URL.name;
...
<DropdownMenu.Root bind:open={show}>
  <DropdownMenu.Trigger class="relative w-[100px]">
    <div class="flex flex-row items-center cursor-pointer w-[100px] mt-3">
      ...
      <span class="text-sm region-text-color ml-1">{$i18n.t("Region")}</span>
    </div>
  </DropdownMenu.Trigger>
  <DropdownMenu.Content class="z-[90] {$mobile ? `w-11/12`: `w-[500px]`}  justify-start border bg-white dark:bg-gray-850 rounded-lg p-3 ml-2"
    transition={flyAndScale} 
    side={$mobile ? "bottom" : "bottom-start"}
    style="border-color: rgba(184, 142, 86, 1)"
    sideOffset={4}>
    <slot>
      <div>
        <div class="region-text-color text-xl text-center">{$i18n.t("Region Setting")}</div>
        <div class="px-5">
          <button class="flex flex-row items-center region-text-color my-2 select-none"
            on:click = { async() => await assignDeUrl("North America") }>
            <CheckRegion checkFlag={ selDeUrl === 'North America'}/>
            <span class="ml-2">{$i18n.t("North America")}</span>
          </button>
          <button class="flex flex-row items-center region-text-color my-2 select-none"
            on:click = { async() => await assignDeUrl("Asia") }>
            <CheckRegion checkFlag={ selDeUrl === 'Asia'}/>
            <span class="ml-2">{$i18n.t("Asia")}</span>
          </button>
          <button class="flex flex-row items-center region-text-color my-2 select-none"
            on:click = { async() => await assignDeUrl("Europe") }>
            <CheckRegion checkFlag={ selDeUrl === 'Europe'}/>
            <span class="ml-2">{$i18n.t("Europe")}</span>
          </button>
          <button class="flex flex-row items-center region-text-color my-2 select-none"
            on:click = { async () => await assignDeUrl("Others") }>
            <CheckRegion checkFlag={ selDeUrl === 'Others'}/>
            <span class="ml-2">{$i18n.t("Others")}</span>
          </button>
          <p class="region-text-color">{$i18n.t("To facilitate a quicker match with the appropriateGPU node, please select your region.")}</p>
        </div>
