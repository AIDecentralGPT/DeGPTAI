								...
								>
									{#if siblings.length > 1}
										<div class="flex justify-start min-w-fit mr-4" dir="ltr">
											...
											{#if message.done}
												<div class="truncate">Generated by {siblings.length} {siblings.length > 1 ? 'LLMs' : 'LLM'}</div>
											{:else}
												<div class="truncate">Generating by {siblings.length} {siblings.length > 1 ? 'LLMs' : 'LLM'}</div>
											{/if}
										</div>
									{/if}
										...
										<div class="flex justify-start min-w-fit mr-4">
											...
											<Tooltip content={$i18n.t('Copy')} placement="bottom">
												<button
													...
													on:click={() => {
														console.log("===================copyToClipboard", message.content);
