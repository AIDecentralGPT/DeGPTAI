    ...
    <div class="w-full overflow-hidden pl-1">
      ...
      <Name>
        ...
        {:else if message?.replytime && checkModelImage(message.model)}
          <span class="text-xs"
            >{$i18n.t('Last for {{ time }} seconds', { time: (message?.replytime - message?.timestamp) % 60 })}</span
          >
        {/if}
                  ...
                  {#each message?.parseInfo?.web ?? [] as item}
                    <div class="flex flex-col rounded-2xl bg-white dark:bg-black mx-2 mb-4 p-4">
                      <div class="flex flex-row">
                        ...
                        <div class="ml-2">
                          <div class="w-[300px] text-sm font-bold line-clamp-1 text-ellipsis">
                      ...
                      <div class="text-xs text-gray-500 w-[300px] line-clamp-3 text-ellipsis mt-1">
                ...
                >
                  ...
                  <div class="self-center w-full">
                    ...
                    {#if $user?.id?.startsWith('0x')}
                      <button
                        class="primaryButton text-white text-sm px-2 py-1 rounded-lg float-right"
                        on:click={() => {
                          $showPriceModal = true;
                        }}
                      >
              ...
              {:else}
                {#each tokens as token, tokenIdx}
                  {#if token.type === 'thinking'}
                    ...
                    <div
                      class="border-l-2 border-slate-200 pl-3 my-2 transition duration-150 {thinkHiden ? 'hidden' : ''}"
                    >
                ...
                {#if message?.error === true}
                  ...
                  >
                    ...
                    <div class="w-full inline-block">
                      <!-- 默认错误输出信息 -->
                      <span class="mr-2"
                        >{$i18n.t('It seems that you are offline. Please reconnect to send messages.')}</span
                      >
                      {#if isLastMessage}
                        <button
                          class="bg-[#B88E56] text-white flex flex-row float-right px-2 rounded-lg"
                          on:click={() => {
                            resentMessage(message?.parentId, true);
                          }}
                        >
