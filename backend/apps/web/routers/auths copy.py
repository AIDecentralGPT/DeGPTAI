...
# async def walletSignIn(form_data: WalletSigninForm):
...
#         # 需要指定ss58_format
#         ss58_format = 42  # 这是默认值

#         # 创建Keypair对象
#         keypair = Keypair(public_key=public_key, ss58_format=ss58_format)

#         # 将nonce从字符串转换为字节数组
#         signed_message = nonce.encode('utf-8')  # 确保与前端转换一致
#         print("signed_message", signed_message)  # 这里不需要调用 hex()

#         # 计算消息哈希
#         message_hash = blake2_256(signed_message)
#         print("message_hash", message_hash.hex())

#         # 验证签名
#         is_valid = keypair.verify(message_hash, signature)

#         print("verify_signature", form_data, is_valid)
